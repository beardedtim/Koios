...
export class NotHealthy extends Error {
  constructor(...services) {
    super()
    this.message = `System not healthy due to the following services: ${services.join(
      ', '
    )}. Please check the logs and fix this issue before trying your request again.`
    this.status = 500
  }
}
...
export class NotAuthorized extends Error {
  constructor() {
    super()
    this.message = `You are not authorized to make that request. Please set the authorization header and try again.`
    this.status = 401
  }
}
