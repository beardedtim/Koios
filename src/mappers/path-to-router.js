...
const get_files_matching_pattern = pattern => new Promise((res, rej) => {
...
const get_routes_from_path = async (dir_path, parentConfig) => {
  ...
  for (const route_path of route_paths) {
    ...
    try {
      ...
      throw new Error('You tried to give me a handler without a config at ' + route_path)
    }
  }
  ...
  const routes = await Promise.all(
    ...
    .then(mods => mods.map(([p, handler]) => {
      const route = path.resolve(p.replace(dir_path, '').replace('/handler.js', '')).split('/')
