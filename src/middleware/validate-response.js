...
const schema_by_status = R.curryN(2, (schemas, status) => R.find(R.propEq('status', status), schemas))
...
export default (config) => (req, res, next) => {
  ...
  res.json = (body) => {
    ...
    if (!schema && !config.sloppy) {
      ...
      try {
        jsonschema.validate(body, { ...R.view(outputSchemaDefintiion, schema), definitions }, { throwAll: true })
        back_res(body)
        return res
      } catch (e) {
